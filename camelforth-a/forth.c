/* forth.c */
/* Thu  6 Jul 02:28:54 UTC 2023 */

/* RESOLVED the EMIT word it is GOOD leave it BE lol */
...
// rescind: extern void putch(char c);
...
CODE(emit) {
    putch(psp++); // WORKED without pointer *psp++ vs psp++
}
...
void make_inner_delay_b() {
    for (volatile int wasting = 4; wasting > 0; wasting--) {
        ;
    }
}
...
CODE(dump) { /* adr n -- */
    unsigned char *p, *q, c, l;
    unsigned int n, i;

    n = *psp++;
    p = (unsigned char *)*psp++;
    const unsigned char spc = ' ';
    const unsigned char dawt = '.';
...
// #define NEWLINE 0x0d /* NEWLINE 0x0a */
...
HEADER(xdump, dots, 0, "\005XDUMP");
HEADER(dump, xdump, 0, "\004DUMP");
HEADER(words, dump, 0, "\005WORDS");
HEADER(resethw, words, 0, "\005RESET");
